...
test("search in the combined graph retains the Main path and relevant sibling providers", () => {
  ...
  const delegationOnly = buildOrchestrationGraph({ profiles, main: { allowedChildren: ["coordinator"] }, ownerId: null,
    layer: "delegation", connectedOnly: true, query: "analyst" });
  assert.deepEqual(new Set(delegationOnly.nodes.map((node) => node.id)), new Set([MAIN_NODE_ID, "coordinator", "analyst"]));
  assert.ok(delegationOnly.edges.every((edge) => edge.kind === "delegation"));
});

test("connected overview hides unreachable profiles without hiding them from search", () => {
  const profiles = [
    agent("coordinator", { allowedChildren: ["reader", "writer"] }),
    agent("reader"), agent("writer"),
    agent("orphan", { allowedChildren: ["reader", "writer"], dependencies: { writer: ["reader"] } }),
  ];
  const input = { profiles, main: { allowedChildren: ["coordinator"] }, ownerId: null,
    layer: "all", connectedOnly: true };
  const overview = buildOrchestrationGraph(input);
  assert.deepEqual(new Set(overview.nodes.map((node) => node.id)),
    new Set([MAIN_NODE_ID, "coordinator", "reader", "writer"]));
  assert.deepEqual(overview.unreachableAgentIds, ["orphan"]);
  assert.ok(overview.edges.every((edge) => edge.ownerId !== "orphan"));
  const dependencies = buildOrchestrationGraph({ ...input, layer: "dependencies" });
  assert.deepEqual(dependencies.nodes.map((node) => node.id), overview.nodes.map((node) => node.id));
  assert.deepEqual(dependencies.edges, []);
  const found = buildOrchestrationGraph({ ...input, query: "orphan" });
  assert.equal(found.matchCount, 1);
  assert.ok(found.nodes.some((node) => node.id === "orphan"));
  assert.deepEqual(found.unreachableAgentIds, ["orphan"]);
  const previousOverview = buildOrchestrationGraph({ ...input, connectedOnly: false });
  assert.ok(previousOverview.nodes.some((node) => node.id === "orphan"));
  assert.deepEqual(previousOverview.unreachableAgentIds, []);
});

test("connected overview ignores an unreachable delegation cycle", () => {
  const profiles = [agent("reachable"), agent("orphan-a", { allowedChildren: ["orphan-b"] }),
    agent("orphan-b", { allowedChildren: ["orphan-a"] })];
  const graph = buildOrchestrationGraph({ profiles, main: { allowedChildren: ["reachable"] },
    ownerId: null, layer: "delegation", connectedOnly: true });
  assert.deepEqual(new Set(graph.nodes.map((node) => node.id)), new Set([MAIN_NODE_ID, "reachable"]));
  assert.deepEqual(graph.unreachableAgentIds, ["orphan-a", "orphan-b"]);
});

test("overview layout groups children by their parents without changing node order", () => {
  const profiles = [
    agent("alpha", { allowedChildren: ["a-one", "d-two"] }),
    agent("beta", { allowedChildren: ["b-one", "c-two"] }),
    ...["a-one", "b-one", "c-two", "d-two"].map((name) => agent(name)),
  ];
  const input = { profiles, main: { allowedChildren: ["alpha", "beta"] },
    ownerId: null, layer: "delegation", connectedOnly: true };
  const first = buildOrchestrationGraph(input);
  const second = buildOrchestrationGraph(input);
  assert.deepEqual(first.nodes, second.nodes);
  const children = first.nodes.filter((node) => ["a-one", "b-one", "c-two", "d-two"].includes(node.id));
  assert.deepEqual(children.map((node) => node.id), ["a-one", "b-one", "c-two", "d-two"]);
  assert.deepEqual([...children].sort((left, right) => left.y - right.y).map((node) => node.id),
    ["a-one", "d-two", "b-one", "c-two"]);
});
...
test("many matches in a 200-agent roster preserve each shortest Main path and graph order", () => {
  const profiles = Array.from({ length: 200 }, (_, index) => agent(`agent-${index}`));
  profiles[0].orchestration = { allowedChildren: Array.from({ length: 20 }, (_, index) => `agent-${10 + index}`) };
  profiles[1].orchestration = { allowedChildren: Array.from({ length: 20 }, (_, index) => `agent-${30 + index}`) };
  profiles[30].orchestration = { allowedChildren: ["agent-150", "agent-0"] };
  const main = { allowedChildren: ["agent-0", "agent-1"] };
  const graph = buildOrchestrationGraph({ profiles, main, ownerId: null, layer: "delegation", query: "agent-1" });
  const effective = effectiveMapProfiles(profiles);
  const matches = effective.filter((profile) => profile.name.includes("agent-1"));
  const visible = new Set([MAIN_NODE_ID, ...matches.flatMap((profile) => mapPathFromMain(profile.name, effective, main))]);
  assert.equal(graph.matchCount, matches.length);
  assert.deepEqual(graph.nodes.map((node) => node.id),
    [MAIN_NODE_ID, ...effective.filter((profile) => visible.has(profile.name)).map((profile) => profile.name)]);
  assert.deepEqual(graph.edges.map(({ source, target }) => [source, target]), [
    [MAIN_NODE_ID, "agent-0"], [MAIN_NODE_ID, "agent-1"],
    ...Array.from({ length: 10 }, (_, index) => ["agent-0", `agent-${10 + index}`]),
    ["agent-1", "agent-30"],
    ["agent-30", "agent-150"], ["agent-30", "agent-0"],
  ]);
});
