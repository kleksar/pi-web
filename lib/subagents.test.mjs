...
test("override detection follows scope precedence case-insensitively", () => {
  ...
  assert.equal(isSubagentProfileOverridden(roster, profiles), false);
  ...
  assert.equal(isSubagentProfileOverridden(project, profiles), true);
  assert.equal(isSubagentProfileOverridden(unrelated, profiles), false);
});

test("repository profile is shared across projects and every edit/deletion appears in Git status", async (t) => {
  const checkout = await mkdtemp(join(tmpdir(), "pi-web-roster-git-"));
  const project = await mkdtemp(join(tmpdir(), "pi-web-roster-project-"));
  const secondProject = await mkdtemp(join(tmpdir(), "pi-web-roster-second-"));
  ...
  t.after(async () => {
    ...
    await Promise.all([checkout, project, secondProject].map((path) => rm(path, { recursive: true, force: true })));
  });
  ...
  const saved = saveSubagentProfile(project, "roster", profile({ name: "shared-agent" }));
  assert.equal(saved.scope, "roster");
  assert.equal(resolveSubagentProfile(secondProject, "shared-agent").scope, "roster");
  assert.match(execFileSync("git", ["-C", checkout, "status", "--short", "--untracked-files=all"], { encoding: "utf8" }),
    /\?\? orchestration\/agents\/shared-agent\.md/);

  await mkdir(join(project, ".pi", "agents"), { recursive: true });
  await mkdir(join(project, ".agents", "agents"), { recursive: true });
  await writeFile(join(project, ".pi", "agents", "shared-agent.md"), "---\nmodel: other/model\n---\nUnsafe override.\n");
  await writeFile(join(project, ".agents", "agents", "shared-agent.md"), "---\nmodel: other/model\n---\nUnsafe override.\n");
  assert.equal(resolveSubagentProfile(project, "shared-agent").scope, "roster");
  assert.throws(() => saveSubagentProfile(project, "project", profile({ name: "shared-agent" })),
    /cannot override a repository agent/);

  const updated = saveSubagentProfile(project, "roster", profile({ name: "shared-agent", enabled: false }));
  assert.equal(updated.enabled, false);
  assert.equal(resolveSubagentProfile(secondProject, "shared-agent"), undefined);
  execFileSync("git", ["-C", checkout, "add", "orchestration/agents/shared-agent.md"]);
  execFileSync("git", ["-C", checkout, "-c", "user.name=Test", "-c", "user.email=test@example.invalid", "commit", "-qm", "profile"]);
  deleteSubagentProfile(project, "roster", "shared-agent");
  assert.match(execFileSync("git", ["-C", checkout, "status", "--short"], { encoding: "utf8" }),
    / D orchestration\/agents\/shared-agent\.md/);
});

test("all shipped roles come from Git and individual Fast changes affect resolved execution", async (t) => {
  const checkout = await mkdtemp(join(tmpdir(), "pi-web-core-roles-git-"));
  const roster = join(checkout, "orchestration");
  const agents = join(roster, "agents");
  const source = new URL("../orchestration/agents/", import.meta.url);
  const previous = process.env.PI_WEB_ROSTER_ROOT;
  t.after(async () => {
    if (previous === undefined) delete process.env.PI_WEB_ROSTER_ROOT;
    else process.env.PI_WEB_ROSTER_ROOT = previous;
  ...
  const names = (await readdir(source)).filter((name) => name.endsWith(".md"));
  assert.equal(names.length, 13);
  for (const name of names) await writeFile(join(agents, name), await readFile(new URL(name, source)));
  execFileSync("git", ["init", "-q", checkout]);
  execFileSync("git", ["-C", checkout, "add", "orchestration/agents"]);
  execFileSync("git", ["-C", checkout, "-c", "user.name=Test", "-c", "user.email=test@example.invalid", "commit", "-qm", "initial"]);
  process.env.PI_WEB_ROSTER_ROOT = roster;

  const visible = listSubagentProfileSources(testAgentDir);
  assert.deepEqual(visible.map((item) => item.name).sort(), ["explore", "general-purpose", "plan"]);
  assert.ok(visible.every((item) => item.scope === "roster"));
  const full = listSubagentProfiles(testAgentDir, { orchestrationEnabled: true });
  assert.equal(full.filter((item) => item.scope === "roster").length, 13);

  const owner = resolveSubagentProfile(testAgentDir, "orchestration-task-owner", { orchestrationEnabled: true });
  const writer = resolveSubagentProfile(testAgentDir, "orchestration-package-writer", { orchestrationEnabled: true });
  assert.equal(owner.fastMode, false);
  assert.equal(writer.fastMode, true);
  assert.equal(saveSubagentProfile(testAgentDir, "roster", { ...owner, fastMode: true }).fastMode, true);
  assert.equal(saveSubagentProfile(testAgentDir, "roster", { ...writer, fastMode: false }).fastMode, false);
  assert.equal(resolveSubagentProfile(testAgentDir, owner.name, { orchestrationEnabled: true }).fastMode, true);
  assert.equal(resolveSubagentProfile(testAgentDir, writer.name, { orchestrationEnabled: true }).fastMode, false);
  assert.match(await readFile(join(agents, `${owner.name}.md`), "utf8"), /pi_web_fast_mode: true/);
  assert.match(await readFile(join(agents, `${writer.name}.md`), "utf8"), /pi_web_fast_mode: false/);
  const status = execFileSync("git", ["-C", checkout, "status", "--short"], { encoding: "utf8" });
  assert.match(status, / M orchestration\/agents\/orchestration-task-owner\.md/);
  assert.match(status, / M orchestration\/agents\/orchestration-package-writer\.md/);
  assert.throws(() => saveSubagentProfile(testAgentDir, "roster", { ...owner, tools: ["bash"] }), /host-owned tools/);
  assert.throws(() => deleteSubagentProfile(testAgentDir, "roster", owner.name), /cannot be deleted/);
  assert.throws(() => saveSubagentProfile(testAgentDir, "global", { ...owner }), /cannot override a repository agent/);

  const edited = (await readFile(join(agents, `${owner.name}.md`), "utf8"))
    .replace(/tools:[\s\S]*?\nload_skills:/, "tools: bash\nload_skills:");
  await writeFile(join(agents, `${owner.name}.md`), edited);
  assert.throws(() => listSubagentProfiles(testAgentDir, { orchestrationEnabled: true }), /host-owned tools/);
});

test("repository profile save never reads through or replaces a symlink", async (t) => {
  const checkout = await mkdtemp(join(tmpdir(), "pi-web-roster-symlink-"));
  const previous = process.env.PI_WEB_ROSTER_ROOT;
  t.after(async () => {
    if (previous === undefined) delete process.env.PI_WEB_ROSTER_ROOT;
    else process.env.PI_WEB_ROSTER_ROOT = previous;
  ...
  const agents = join(checkout, "orchestration", "agents");
  await mkdir(agents, { recursive: true });
  await writeFile(join(checkout, "private.md"), "outside\n");
  await symlink(join(checkout, "private.md"), join(agents, "linked.md"));
  execFileSync("git", ["init", "-q", checkout]);
  process.env.PI_WEB_ROSTER_ROOT = join(checkout, "orchestration");
  assert.throws(() => saveSubagentProfile(checkout, "roster", profile({ name: "linked" })),
    /must be a regular file/);
  assert.throws(() => deleteSubagentProfile(checkout, "roster", "linked"),
    /must be a regular file/);
  assert.equal(await readFile(join(checkout, "private.md"), "utf8"), "outside\n");
});

test("an oversized project profile cannot force an unbounded read during discovery", async (t) => {
  const project = await mkdtemp(join(tmpdir(), "pi-web-large-profile-"));
  t.after(() => rm(project, { recursive: true, force: true }));
  await mkdir(join(project, ".pi", "agents"), { recursive: true });
  await writeFile(join(project, ".pi", "agents", "oversized.md"), "x".repeat(512 * 1024 + 1));
  assert.equal(listSubagentProfileSources(project).some((item) => item.name === "oversized"), false);
  assert.throws(() => saveSubagentProfile(project, "project", profile({ name: "oversized" })), /exceeds.*limit/);
});
