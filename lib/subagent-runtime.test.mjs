...
const { readSubagentSessionResources, resolveSubagentProfile } = await jiti.import("./subagents.ts");
...
async function liveControllerFixture(t, profiles = {}, fixtureOptions = {}) {
  ...
  const fakeModel = fixtureOptions.model ?? { provider: "test", id: "no-network" };
  ...
  const root = {
    ...
    inner: {
      sessionManager: rootManager, modelRuntime: fixtureOptions.modelRuntime ?? { getModel: () => undefined, getModels: () => [] },
  ...
  const controller = createSubagentController({
    ...
    async createFromServices({ sessionManager, services, model }) {
      ...
      const inner = {
        ...
        modelRuntime: root.inner.modelRuntime, model: model ?? fakeModel,
        agent: { state: { thinkingLevel: "off" } },
        subscribe: (callback) => { onEvent = callback; return () => { onEvent = () => {}; }; },
        steer: async () => {}, getLastAssistantText: () => fixtureOptions.turnEndsBeforeComplete ? "unfinished partial answer" : "done",
        __testRunning: () => running,
        __testCwd: services.__testCwd,
        prompt: async () => {
          running = true;
          await new Promise((resolve) => { unblock = resolve; blockers.set(id, resolve); });
          for (let index = 0; index < (fixtureOptions.turnEndsBeforeComplete ?? 0); index += 1) {
            onEvent({ type: "turn_start" });
            const finalAnswer = fixtureOptions.finishAtGraceTurn && index === fixtureOptions.turnEndsBeforeComplete - 1;
            onEvent({ type: "turn_end", message: { role: "assistant", stopReason: finalAnswer ? "stop" : "toolUse",
              content: finalAnswer ? [{ type: "text", text: "Final answer" }] : [{ type: "toolCall", id: "call", name: "read", arguments: {} }] } });
          }
          if (fixtureOptions.continueBeyondTurnLimit) onEvent({ type: "turn_start" });
          running = false;
          if (fixtureOptions.promptFailureAfterTurnLimit) throw new Error("Agent loop stopped at turn limit");
        },
        abort: async () => {
...
test("reaching max_turns reports failure instead of publishing an unfinished result", async (t) => {
  for (const promptFailureAfterTurnLimit of [false, true]) {
    const fixture = await liveControllerFixture(t, {}, {
      turnEndsBeforeComplete: 1, continueBeyondTurnLimit: true, promptFailureAfterTurnLimit,
    });
    try {
      const started = await fixture.start(fixture.rootId, "explore", 1, { maxTurns: 1 });
      fixture.blockers.get(started.run.sessionId)();
      const result = await started.completion;
      assert.equal(result.status, "failed", `prompt ${promptFailureAfterTurnLimit ? "rejects" : "resolves"}`);
      assert.match(result.error, /turn limit/i);
      assert.equal(result.result, undefined);
      const persisted = fixture.sessions.get(result.sessionId).inner.sessionManager.getEntries()
        .filter((entry) => entry.customType === "pi-web:subagent-result").at(-1).data;
...
test("an explicit Stop takes precedence over a turn-limit abort", async (t) => {
  const fixture = await liveControllerFixture(t, {}, { turnEndsBeforeComplete: 1, continueBeyondTurnLimit: true });
  try {
    const started = await fixture.start(fixture.rootId, "explore", 1, { maxTurns: 1 });
    await fixture.controller.abort(started.run.sessionId);
    const result = await started.completion;
...
test("a complete final answer on the last permitted turn succeeds without another model request", async (t) => {
  const fixture = await liveControllerFixture(t, {}, { turnEndsBeforeComplete: 1, finishAtGraceTurn: true });
  try {
    const started = await fixture.start(fixture.rootId, "explore", 1, { maxTurns: 1 });
    fixture.blockers.get(started.run.sessionId)();
    const result = await started.completion;
...
test("a resumed agent retains its pinned max_turns limit for each new invocation", async (t) => {
  const options = { turnEndsBeforeComplete: 1, finishAtGraceTurn: true };
  const fixture = await liveControllerFixture(t, {}, options);
  try {
    const started = await fixture.start(fixture.rootId, "explore", 1, { maxTurns: 1 });
    fixture.blockers.get(started.run.sessionId)();
    assert.equal((await started.completion).status, "completed");
    const child = fixture.sessions.get(started.run.sessionId).inner;
    assert.equal(readSubagentSessionResources(child.sessionManager.getEntries()).maxTurns, 1);
    // The test double does not write model messages; persist its JSONL transcript
    // explicitly to exercise the same snapshot an actual SessionManager reopens.
    await writeFile(started.run.sessionPath, [child.sessionManager.getHeader(), ...child.sessionManager.getEntries()]
      .map((entry) => JSON.stringify(entry)).join("\n") + "\n");
    assert.equal(readSubagentSessionResources(SessionManager.open(started.run.sessionPath).getEntries()).maxTurns, 1);

    const resume = (label) => fixture.controller.extensionRuntime.resume({
      parentContext: fixture.sessions.get(fixture.rootId).inner,
      parentToolCallId: label, sessionId: started.run.sessionId,
      task: "Continue", description: "Continue",
    });
    const successfulResume = await resume("resume-one-complete-turn");
    fixture.blockers.get(started.run.sessionId)();
    assert.equal((await successfulResume.completion).status, "completed",
      "restored history must not consume this invocation's budget");

    options.finishAtGraceTurn = false;
    options.continueBeyondTurnLimit = true;
    const resumed = await resume("resume-exceeds-original-limit");
    fixture.blockers.get(started.run.sessionId)();
...
test("an unsafe or subturn max_turns override is rejected before creating a session snapshot", async (t) => {
  const fixture = await liveControllerFixture(t);
  try {
    for (const maxTurns of [0.5, Number.MAX_SAFE_INTEGER + 1]) {
      await assert.rejects(fixture.start(fixture.rootId, "explore", 1, { maxTurns }), /max_turns.*safe turn/);
    }
...
test("Fast coordinator requests priority while its builtin child stays normal, and its snapshot remains pinned", async (t) => {
  const model = { provider: "openai-codex", api: "openai-codex-responses", id: "gpt-5.5" };
  const fixture = await liveControllerFixture(t, { coordinator1: "explore" }, { model });
  const file = join(fixture.cwd, ".pi", "agents", "coordinator1.md");
  await writeFile(file, (await readFile(file, "utf8")).replace("run_in_background: false", "run_in_background: false\npi_web_fast_mode: true"));
  const before = profileAuthorityPin(resolveSubagentProfile(fixture.cwd, "coordinator1"));
  try {
    const coordinator = await fixture.start(fixture.rootId, "coordinator1", 1);
    const inner = fixture.sessions.get(coordinator.run.sessionId).inner;
    const resources = readSubagentSessionResources(inner.sessionManager.getEntries());
    assert.equal(resources.fastMode, true);
    assert.equal((await inner.agent.onPayload({ model: "gpt-5.5" }, model)).service_tier, "priority");

    const specialist = await fixture.start(coordinator.run.sessionId, "explore", 2);
    const specialistInner = fixture.sessions.get(specialist.run.sessionId).inner;
    assert.equal(readSubagentSessionResources(specialistInner.sessionManager.getEntries()).fastMode, false);
    assert.equal(specialistInner.agent.onPayload, undefined);

    await writeFile(file, (await readFile(file, "utf8")).replace("pi_web_fast_mode: true", "pi_web_fast_mode: false"));
    assert.notEqual(profileAuthorityPin(resolveSubagentProfile(fixture.cwd, "coordinator1")).sha256, before.sha256);
    assert.equal(readSubagentSessionResources(inner.sessionManager.getEntries()).fastMode, true);
...
test("default-off Fast mode retains pre-feature authority fingerprints", () => {
  const oldProfile = { name: "reader", scope: "builtin", tools: [], enabled: true };
  assert.equal(profileAuthorityPin({ ...oldProfile, fastMode: false }).sha256, profileAuthorityPin(oldProfile).sha256);
  assert.notEqual(profileAuthorityPin({ ...oldProfile, fastMode: true }).sha256, profileAuthorityPin(oldProfile).sha256);
});

test("an unsupported Fast profile fails before allocating a child session or loading resources", async (t) => {
  const fixture = await liveControllerFixture(t, { coordinator1: "explore" });
  const file = join(fixture.cwd, ".pi", "agents", "coordinator1.md");
  await writeFile(file, (await readFile(file, "utf8")).replace("run_in_background: false", "run_in_background: false\npi_web_fast_mode: true"));
  try {
    await assert.rejects(fixture.start(fixture.rootId, "coordinator1", 1), /Fast mode.*requires an OpenAI/);
    assert.equal(fixture.sessions.size, 1);
...
test("extension models resolve after child services load for normal and Fast agents", async (t) => {
  const normal = { provider: "local-test", api: "anthropic-messages", id: "extension-normal" };
  const fast = { provider: "openai-codex", api: "openai-codex-responses", id: "extension-fast" };
  const available = new Map();
  const runtime = {
    getModel: (provider, id) => available.get(`${provider}/${id}`),
    getModels: () => [...available.values()],
  };
  let loads = 0;
  const fixture = await liveControllerFixture(t,
    { coordinator1: "explore", coordinator2: "explore" },
    { modelRuntime: runtime, beforeServicesReturn: () => {
      const model = ++loads === 1 ? normal : fast;
      available.set(`${model.provider}/${model.id}`, model);
    } });
  const firstFile = join(fixture.cwd, ".pi", "agents", "coordinator1.md");
  const secondFile = join(fixture.cwd, ".pi", "agents", "coordinator2.md");
  await writeFile(firstFile, (await readFile(firstFile, "utf8")).replace("run_in_background: false", "run_in_background: false\nmodel: local-test/extension-normal"));
  await writeFile(secondFile, (await readFile(secondFile, "utf8")).replace("run_in_background: false", "run_in_background: false\nmodel: openai-codex/extension-fast\npi_web_fast_mode: true"));
  try {
    assert.equal(runtime.getModel(normal.provider, normal.id), undefined);
    const regular = await fixture.start(fixture.rootId, "coordinator1", 1);
    assert.equal(fixture.sessions.get(regular.run.sessionId).inner.model.id, normal.id);
    assert.equal(fixture.sessions.get(regular.run.sessionId).inner.agent.onPayload, undefined);
    assert.equal(runtime.getModel(fast.provider, fast.id), undefined);
    const accelerated = await fixture.start(fixture.rootId, "coordinator2", 2);
    const acceleratedInner = fixture.sessions.get(accelerated.run.sessionId).inner;
    assert.equal(acceleratedInner.model.id, fast.id);
    assert.equal((await acceleratedInner.agent.onPayload({}, fast)).service_tier, "priority");
...
test("a third-level orchestrator has no delegation tools and a fourth child is denied before services", async (t) => {
  ...
  try {
    for (const name of ["coordinator2", "coordinator3"]) {
      const file = join(fixture.cwd, ".pi", "agents", `${name}.md`);
      await writeFile(file, (await readFile(file, "utf8")).replace("run_in_background: false", "run_in_background: true"));
    }
    ...
    assert.equal(second.run.runInBackground, false, "nested profile default must run in foreground");
    assert.equal(third.run.runInBackground, false, "the third level must also run in foreground");
    ...
    assert.equal(fixture.serviceCalls, 3);
    await fixture.controller.abort(third.run.sessionId);
    await third.completion;
    const resumed = await fixture.controller.extensionRuntime.resume({
      parentContext: fixture.sessions.get(second.run.sessionId).inner, parentToolCallId: "resume-third",
      sessionId: third.run.sessionId, task: "Continue", description: "Continue",
    });
    assert.equal(resumed.run.runInBackground, false, "resume also ignores the profile's background default");
    await fixture.controller.abort(resumed.run.sessionId);
...
test("Main dependency checks also govern a direct child's resume in the current user turn", async (t) => {
  const fixture = await liveControllerFixture(t);
  const profiles = Object.fromEntries(["plan", "explore"].map((name) =>
    [name, profileAuthorityPin(resolveSubagentProfile(fixture.cwd, name))]));
  fixture.rootManager.appendCustomEntry("pi-web:main-resources", {
    version: 1,
    orchestration: {
      allowedChildren: ["plan", "explore"],
      dependencies: { explore: ["plan"] },
      childProfiles: profiles,
    },
  });
  const resume = (sessionId) => fixture.controller.extensionRuntime.resume({
    parentContext: fixture.sessions.get(fixture.rootId).inner,
    parentToolCallId: `resume-${sessionId}`, sessionId,
    task: "continue", description: "Continue",
  });
  try {
    const producer = await fixture.start(fixture.rootId, "plan", 1);
    fixture.blockers.get(producer.run.sessionId)();
    assert.equal((await producer.completion).status, "completed");

    const consumer = await fixture.start(fixture.rootId, "explore", 2);
    fixture.blockers.get(consumer.run.sessionId)();
    assert.equal((await consumer.completion).status, "completed");

    const producerRetry = await fixture.start(fixture.rootId, "plan", 3);
    await assert.rejects(resume(consumer.run.sessionId), /Missing dependency result from plan for explore/);
    assert.equal(globalThis.__piSubagentRootAdmissions?.get(fixture.rootId), 1,
      "rejected resume must release its admission");
    fixture.blockers.get(producerRetry.run.sessionId)();
    assert.equal((await producerRetry.completion).status, "completed");

    const resumed = await resume(consumer.run.sessionId);
    fixture.blockers.get(consumer.run.sessionId)();
    assert.equal((await resumed.completion).status, "completed");
    assert.equal(resumed.run.sessionId, consumer.run.sessionId);

    fixture.rootManager.appendMessage({ role: "user", content: "new task", timestamp: Date.now() });
    const before = fixture.sessions.get(consumer.run.sessionId).inner.sessionManager.getEntries().length;
    await assert.rejects(resume(consumer.run.sessionId), /belongs to a previous orchestrator invocation/);
    assert.equal(fixture.sessions.get(consumer.run.sessionId).inner.sessionManager.getEntries().length, before,
      "a previous-turn child must not record a new run");
    assert.equal(globalThis.__piSubagentRootAdmissions?.get(fixture.rootId), undefined);
  } finally {
    await fixture.cleanup();
  }
...
test("an unsupported extension Fast model leaves the previous dependency artifact available", async (t) => {
  const unsupported = { provider: "extension", api: "anthropic-messages", id: "unsupported-fast" };
  let loads = 0;
  const modelRuntime = {
    getModel: (provider, id) => loads >= 2 && provider === unsupported.provider && id === unsupported.id
      ? unsupported : undefined,
    getModels: () => loads >= 2 ? [unsupported] : [],
  };
  const fixture = await liveControllerFixture(t, {}, {
    modelRuntime,
    beforeServicesReturn: () => { loads += 1; },
  });
  await writeFile(join(fixture.cwd, ".pi", "agents", "explore.md"),
    "---\nname: explore\ndescription: Fast explorer\ntools: none\nload_skills: false\nload_extensions: false\n" +
    "model: extension/unsupported-fast\npi_web_fast_mode: true\n---\nExplore.\n");
  const profiles = Object.fromEntries(["plan", "explore"].map((name) =>
    [name, profileAuthorityPin(resolveSubagentProfile(fixture.cwd, name))]));
  fixture.rootManager.appendCustomEntry("pi-web:main-resources", {
    version: 1,
    orchestration: {
      allowedChildren: ["plan", "explore"], dependencies: { explore: ["plan"] }, childProfiles: profiles,
    },
  });
  try {
    const producer = await fixture.start(fixture.rootId, "plan", 1);
    fixture.blockers.get(producer.run.sessionId)();
    assert.equal((await producer.completion).status, "completed");
    const markersBefore = structuredClone(fixture.rootManager.getEntries());
    assert.equal(markersBefore.some((entry) => entry.customType === "pi-web:subagent-artifact"), true);
    assert.equal(modelRuntime.getModel(unsupported.provider, unsupported.id), undefined);

    await assert.rejects(fixture.start(fixture.rootId, "explore", 2), /Fast mode.*requires an OpenAI/);
    assert.equal(loads, 2, "the extension registers its model during child service loading");
    assert.deepEqual(fixture.rootManager.getEntries(), markersBefore,
      "a rejected first run must not invalidate the producer's result");
    assert.equal(fixture.sessions.size, 2, "no consumer session was created");
    assert.equal(globalThis.__piSubagentRootAdmissions?.get(fixture.rootId), undefined);
  } finally {
    await fixture.cleanup();
  }
...
test("unsupported model on Fast resume leaves dependency artifacts untouched and never prompts", async (t) => {
  const model = { provider: "openai-codex", api: "openai-codex-responses", id: "gpt-5.5" };
  const fixture = await liveControllerFixture(t, {}, { model });
  await writeFile(join(fixture.cwd, ".pi", "agents", "explore.md"),
    "---\nname: explore\ndescription: Fast explorer\ntools: none\nload_skills: false\nload_extensions: false\npi_web_fast_mode: true\n---\nExplore.\n");
  const profiles = Object.fromEntries(["plan", "explore"].map((name) =>
    [name, profileAuthorityPin(resolveSubagentProfile(fixture.cwd, name))]));
  ...
  try {
    ...
    fixture.blockers.get(consumer.run.sessionId)();
    assert.equal((await consumer.completion).status, "completed");
    const childInner = fixture.sessions.get(consumer.run.sessionId).inner;
    assert.equal(readSubagentSessionResources(childInner.sessionManager.getEntries()).fastMode, true);

    const markersBefore = structuredClone(fixture.rootManager.getEntries());
    const childEntriesBefore = structuredClone(childInner.sessionManager.getEntries());
    let prompts = 0;
    const originalPrompt = childInner.prompt;
    childInner.prompt = async (...args) => { prompts += 1; return originalPrompt(...args); };
    childInner.model = { provider: "anthropic", api: "anthropic-messages", id: "unsupported" };

    await assert.rejects(fixture.controller.extensionRuntime.resume({
      parentContext: fixture.sessions.get(fixture.rootId).inner,
      parentToolCallId: "resume-unsupported-fast", sessionId: consumer.run.sessionId,
      task: "Continue", description: "Continue",
    }), /Fast mode.*requires an OpenAI/);
    assert.equal(prompts, 0);
    assert.deepEqual(fixture.rootManager.getEntries(), markersBefore,
      "a rejected resume must not consume or invalidate the producer's result");
    assert.deepEqual(childInner.sessionManager.getEntries(), childEntriesBefore,
      "a rejected resume must not start a new child attempt");
    assert.equal(globalThis.__piSubagentRootAdmissions?.get(fixture.rootId), undefined);
  } finally {
    await fixture.cleanup();
  }
