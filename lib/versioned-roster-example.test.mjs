...
test("shipped roster works in a different project and keeps specialists isolated", async (t) => {
  ...
  assert.deepEqual(coordinator?.orchestration?.allowedChildren, [
    "project-policy-reader", "project-requirements-reader", "project-docs-reader", "project-code-reader", "git-reader",
  ...
  assert.equal(byName.has("github-reader"), false);
  assert.deepEqual(byName.get("git-reader")?.tools, ["github_read", "git_read"]);
  assert.equal(byName.get("git-reader")?.loadExtensions, false);
  ...
  assert.equal(complex?.orchestration?.dependencies?.["implementation-coordinator"], undefined);
