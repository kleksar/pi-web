...
test("an orchestrator with no file tools stays in the extension-capable prompt pipeline", () => {
  const plan = buildSubagentPromptPlan({
    profileSystemPrompt: "Route permitted work.",
    tools: [],
    loadSkills: false,
    loadExtensions: false,
    canDelegate: true,
    task: "Delegate the review.",
    inheritedParentContext: "Parent context",
  });

  assert.equal(plan.chatOnly, false);
  assert.equal(plan.exactSystemPrompt, undefined);
  assert.deepEqual(plan.appendSystemPrompt, ["Route permitted work.", "Parent context"]);
  assert.equal(plan.delegatedTask, "Delegate the review.");
});
