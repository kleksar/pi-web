...
export const zhTWLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "agents.showOrchestrationProfiles": "瀏覽編排代理設定檔",
    ...
    "agents.fastModeDescription": "為支援此模式的 Luna 代理使用優先處理。工作階段費用估算可能不含快速模式的附加費用。",
    ...
    "agentSwitcher.expand": "展開代理執行紀錄",
    "agentSwitcher.collapse": "收合代理執行紀錄",
    ...
    "session.cost": "費用",
    "session.agentCost": "代理費用估算",
    "session.familyCost": "工作階段與子代理費用估算",
    "session.taskCost": "任務代理費用估算",
    "session.taskCostHelp": "任務費用僅包含關聯到該任務的代理；主代理的費用由多個任務共用。",
    "session.partialCost": "僅為部分費用；部分代理的費用未知",
    "session.familyCostHelp": "包含主代理及其子代理。分支工作階段單獨計算；實際費用以服務供應商帳單為準。",
    "session.context": "上下文",
    ...
    "chat.mainDispatcher": "Luna 調度代理",
    "chat.mainDispatcherHint": "新工作階段：Luna High Fast 透過 Agent 工具分派任務。需啟用 Pi Web 內建子代理。",
