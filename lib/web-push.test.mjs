...
test("notifySessionComplete sends English payloads to subscriptions with older locale settings", async () => {
...
test("notifySessionComplete uses the English generic title for old subscriptions", async () => {
...
test("push payload text is English", () => {
  assert.equal(localeText("taskFinished"), "Task finished.");
  assert.equal(localeText("sessionComplete"), "Session complete");
});
