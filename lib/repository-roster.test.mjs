...
test("skill directory links cannot make SDK discovery recurse through an in-root cycle", async (t) => {
  const { roster } = await rosterFixture(t);
  const skills = join(roster, "skills");
  await symlink(skills, join(skills, "cycle-a"), "dir");
  await symlink(skills, join(skills, "cycle-b"), "dir");
  assert.throws(() => getRepositorySkillPaths(), /directory symlinks are not supported/);
});
