...
test("project overrides inherit absent fields and explicitly clear inherited assignments", () => {
  ...
  assert.deepEqual(mergeMainAgentConfigs(global, { allowedBuiltInTools: ["read"] }), {
    ...global, allowedBuiltInTools: ["read"],
  });
  assert.deepEqual(changedProjectMainOverrides(global, { ...global, allowedBuiltInTools: ["read"] },
    {}, global), { allowedBuiltInTools: ["read"] });
  assert.deepEqual(changedProjectMainOverrides({ ...global, allowedBuiltInTools: ["read"] }, global,
    { allowedBuiltInTools: ["read"] }, global), {});
...
test("Main config refuses invalid dependency links and a damaged existing file", async (t) => {
  ...
  for (const invalid of [["Agent"], ["read", "read"], ["read", "unknown"], "read", null]) {
    assert.throws(() => validateMainAgentConfig({ allowedBuiltInTools: invalid }),
      /Allowed Main built-in tools/);
  }
