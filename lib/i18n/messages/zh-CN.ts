...
export const zhCNLocale: LocalePlugin = {
  ...
  messages: {
    ...
    "agents.showOrchestrationProfiles": "浏览编排代理配置",
    ...
    "agents.fastModeDescription": "为支持该模式的 Luna 代理使用优先处理。会话费用估算可能不包含快速模式的附加费用。",
    ...
    "agentSwitcher.expand": "展开代理运行记录",
    "agentSwitcher.collapse": "收起代理运行记录",
    ...
    "session.cost": "费用",
    "session.agentCost": "代理费用估算",
    "session.familyCost": "会话及子代理费用估算",
    "session.taskCost": "任务代理费用估算",
    "session.taskCostHelp": "任务费用仅包含关联到该任务的代理；主代理的费用由多个任务共享。",
    "session.partialCost": "仅为部分费用；某些代理的费用未知",
    "session.familyCostHelp": "包含主代理及其子代理。分叉会话单独计算；实际费用以服务提供方账单为准。",
    "session.context": "上下文",
    ...
    "chat.mainDispatcher": "Luna 调度代理",
    "chat.mainDispatcherHint": "新会话：Luna High Fast 通过 Agent 工具分派任务。需启用 Pi Web 内置子代理。",
