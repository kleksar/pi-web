...
test("an in-project file link resolves to its real file before secure reading", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-subagent-linked-input-"));
  try {
    await writeFile(join(cwd, "evidence.md"), "Local evidence", "utf8");
    await symlink(join(cwd, "evidence.md"), join(cwd, "linked.md"));
    assert.deepEqual(loadSubagentInputFiles(cwd, ["linked.md"]), [
      { path: "evidence.md", content: "Local evidence" },
    ]);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
...
test("a large sparse Agent input is rejected before allocating its contents", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-subagent-sparse-input-"));
  try {
    const file = await open(join(cwd, "large.txt"), "w");
    try {
      await file.truncate(MAX_SUBAGENT_INPUT_BYTES * 128);
    } finally {
      await file.close();
    }
    assert.throws(() => loadSubagentInputFiles(cwd, ["large.txt"]), /total limit/);
    await writeFile(join(cwd, "small.txt"), "Targeted context\n");
    assert.deepEqual(loadSubagentInputFiles(cwd, ["small.txt"]), [
      { path: "small.txt", content: "Targeted context\n" },
    ]);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
