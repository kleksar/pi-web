...
const directParentContext = { sessionManager: { getSessionId: () => "parent-session" } };
...
test("integrated extension exposes the legacy-compatible tool names", async () => {
  ...
  assert.deepEqual([...tools.keys()], ["Agent", "get_subagent_result", "get_subagent_results", "steer_subagent"]);
...
test("integrated extension registers no tools while its feature is disabled", async () => {
  ...
  assert.deepEqual([...enabledTools.keys()], ["Agent", "get_subagent_result", "get_subagent_results", "steer_subagent"]);
});
...
test("Main cannot block on a running owner while owner may wait for its readers", async () => {
  const owner = run({ profile: "orchestration-task-owner", status: "running", runInBackground: true });
  const reader = run({ profile: "orchestration-code-reader", status: "running", runInBackground: true });
  let collected = 0;
  const { tools } = await loadTools({
    async get() { return owner; },
    async collect() { collected++; return [reader]; },
  });
  const mainCtx = { sessionManager: {
    getSessionId: () => owner.parentSessionId,
    getEntries: () => [{ type: "custom", customType: "pi-web:main-dispatcher", data: { version: 1, enabled: true } }],
  } };
  const one = await tools.get("get_subagent_result").execute("one", { agent_id: owner.sessionId, wait: true }, undefined, undefined, mainCtx);
  const many = await tools.get("get_subagent_results").execute("many", { agent_ids: [owner.sessionId], wait: true }, undefined, undefined, mainCtx);
  assert.equal(one.isError, true);
  assert.equal(many.isError, true);
  assert.match(one.content[0].text, /cannot wait for a running task owner/);
  assert.equal(collected, 0);

  const polling = await tools.get("get_subagent_result").execute("poll", { agent_id: owner.sessionId, wait: false }, undefined, undefined, mainCtx);
  assert.equal(polling.isError, undefined);
  const childCtx = { sessionManager: { getSessionId: () => reader.parentSessionId } };
  const ownerWaits = await tools.get("get_subagent_results").execute("readers", { agent_ids: [reader.sessionId], wait: true }, undefined, undefined, childCtx);
  assert.equal(ownerWaits.isError, undefined);
...
test("get_subagent_result covers missing, running, completed, failed, and aborted runs", async () => {
  ...
  let result = await tool.execute("call", { agent_id: "missing" }, undefined, undefined, directParentContext);
  assert.equal(result.isError, true);
  assert.match(result.content[0].text, /not found/);

  current = run();
  result = await tool.execute("call", { agent_id: "child-session" }, undefined, undefined, directParentContext);
...
test("get_subagent_result wait honors abort signals", async () => {
  ...
  await assert.rejects(
    tools.get("get_subagent_result").execute("call", { agent_id: "child-session", wait: true }, controller.signal, undefined, directParentContext),
    /Result wait aborted/,
  );
});
...
test("steer_subagent returns success and runtime failures", async () => {
  ...
  const success = await tool.execute("call", { agent_id: "child-session", message: "focus" }, undefined, undefined, directParentContext);
  assert.deepEqual(calls[0], ["child-session", "focus"]);
  assert.match(success.content[0].text, /sent/);

  const failure = await tool.execute("call", { agent_id: "child-session", message: "fail" }, undefined, undefined, directParentContext);
  assert.equal(failure.isError, true);
...
test("collecting a finished background result marks it consumed so the completion notification is skipped", async () => {
  ...
  const result = await tools.get("get_subagent_result").execute("call", { agent_id: "child-session" }, undefined, undefined, directParentContext);
...
test("a background result that is still running is not marked consumed", async () => {
  ...
  await tools.get("get_subagent_result").execute("call", { agent_id: "child-session" }, undefined, undefined, directParentContext);
