...
test("a project overview routes from Main through evidence to docs and code readers", async (t) => {
  const { profiles, byName, main } = await loadedRoster(t);
  const evidence = requireAgent(byName, "evidence-coordinator");
  assert.deepEqual(main.allowedBuiltInTools, [], "Main must not inspect project files itself");
  assert.deepEqual(main.selectedExtensionTools, [], "Main must not load unrelated extension tools");
  assert.ok(main.orchestration.allowedChildren.includes(evidence.name));
  assert.deepEqual(evidence.tools, []);
  for (const reader of ["project-docs-reader", "project-code-reader"]) {
    assert.ok(evidence.orchestration.allowedChildren.includes(reader));
  }
  const edges = buildOrchestrationGraph({ profiles, main: main.orchestration,
    ownerId: null, layer: "delegation" }).edges;
  assert.ok(edges.some((edge) => edge.ownerId === MAIN_NODE_ID && edge.target === evidence.name));
  assert.ok(edges.some((edge) => edge.ownerId === evidence.name && edge.target === "project-docs-reader"));
  assert.ok(edges.some((edge) => edge.ownerId === evidence.name && edge.target === "project-code-reader"));
});
