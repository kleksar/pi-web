...
const { AgentSessionWrapper, readMainDispatcherSessionConfig } = await jiti.import("./rpc-manager.ts");

test("Main dispatcher reopens with the creation-time model, effort and Fast settings", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-dispatcher-session-"));
  try {
    const sessions = join(cwd, "sessions");
    await mkdir(sessions);
    const manager = SessionManager.create(cwd, sessions);
    const pinned = { model: "openai-codex/gpt-6-luna", thinking: "high", fastMode: true,
      additionalInstructions: "Keep exact user corrections." };
    manager.appendCustomEntry("pi-web:main-dispatcher", { version: 1, enabled: true, config: pinned });
    manager.appendMessage({ role: "user", content: "Test creation-time bindings", timestamp: Date.now() });
    // The SDK flushes a new session file only after its first assistant message.
    ...
    assert.deepEqual(readMainDispatcherSessionConfig(reopened.getEntries()), pinned);
    assert.deepEqual(readMainDispatcherSessionConfig([{ type: "custom", customType: "pi-web:main-dispatcher",
      data: { version: 1, enabled: true } }]).additionalInstructions, "");

    const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
    const startup = source.slice(source.indexOf("export async function startRpcSession"));
    assert.match(startup, /readMainDispatcherSessionConfig\(existingEntries\)/);
    assert.match(startup, /appendCustomEntry\(MAIN_DISPATCHER_ENTRY_TYPE, \{ version: 1, enabled: true, config: dispatcherConfig \}\)/);
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
});

test("dispatcher rejects direct shell commands at the RPC boundary", async (t) => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-dispatcher-shell-"));
  t.after(() => rm(cwd, { recursive: true, force: true }));
  const manager = SessionManager.create(cwd, cwd);
  manager.appendCustomEntry("pi-web:main-dispatcher", { version: 1, enabled: true });
  const wrapper = new AgentSessionWrapper({
    ...
    extensionRunner: {}, agent: { state: {} },
    executeBash() { throw new Error("shell must not run"); },
    dispose() {},
  });
  try {
    await assert.rejects(wrapper.send({ type: "bash", command: "pwd" }), /direct shell access/);
  } finally {
    wrapper.destroy();
  }
});
...
test("built-in subagents persist their selected resource policy", async () => {
  ...
  assert.match(subagentSource, /excludeTools: canDelegate \? \[\] : \[\.\.\.SUBAGENT_CONTROL_TOOL_NAMES\]/);
  ...
  assert.match(startupSource, /resourceLoaderOptions: mainDispatcher[\s\S]*?: subagentResources/);
  ...
  assert.match(startupSource, /excludeTools: subagentResources\.allowedSubagents\?\.length \? \[\] : \[\.\.\.SUBAGENT_CONTROL_TOOL_NAMES\]/);
...
test("exact prompts are sent through before_agent_start instead of the SDK prompt state", async () => {
  ...
  assert.match(subagentSource, /createExactSystemPromptExtension\(\(\) => promptPlan\.exactSystemPrompt\)/);
...
test("normal sessions restore persisted tool selections before loading resources", async () => {
  ...
  assert.match(startupSource, /const selectedToolNames = mainDispatcher[\s\S]*?: subagentResources\?\.tools \?\? persistedToolNames \?\? requestedToolNames/);
  ...
  assert.match(startupSource, /const trustReloadOptions = mainDispatcher[\s\S]*?: subagentResources[\s\S]*?subagentLoadsResources[\s\S]*?projectTrustReloadOptions\(sessionCwd, agentDir\)/);
