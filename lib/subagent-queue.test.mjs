...
test("an owner awaiting readers does not occupy their task worker permits", async () => {
  const queue = new SubagentQueue();
  let finishOwner;
  const owner = queue.enqueue("task-a:owners", 1,
    () => new Promise((resolve) => { finishOwner = resolve; }), () => {});
  let leafStarted = false;
  const leaf = queue.enqueue("task-a:workers", 1, async () => {
    leafStarted = true;
    return "source found";
  }, () => {});

  assert.equal(await leaf.promise, "source found");
  assert.equal(leafStarted, true);
  finishOwner("decision ready");
  assert.equal(await owner.promise, "decision ready");
});
