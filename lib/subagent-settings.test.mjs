...
test("oversized local subagent settings cannot exhaust memory during discovery", async (t) => {
  const root = await mkdtemp(join(tmpdir(), "pi-web-large-subagent-settings-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  const dir = join(root, "agents");
  await mkdir(dir);
  const file = await open(join(dir, "settings.json"), "w");
  try { await file.truncate(64 * 1024 * 1024); } finally { await file.close(); }
  assert.throws(() => readSubagentSettings(join(dir, "settings.json")), /limit/);
});
