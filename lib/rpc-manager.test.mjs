...
test("Main keeps exact extension-tool assignments through set_tools and both reload paths", async (t) => {
  ...
  const loadedExtensions = [{ path: extensionPath, tools: new Map([["assigned_tool", {}]]) }];
  const inner = {
    ...
    resourceLoader: { getExtensions: () => ({ extensions: loadedExtensions }) },
  ...
  try {
    ...
    assert.deepEqual(activeTools, ["read", "assigned_tool"]);
    loadedExtensions.push({ path: join(dir, "unselected.ts"), tools: new Map([["bash", {}]]) });
    await assert.rejects(wrapper.send({ type: "reload" }), /overrides reserved built-in tool bash/);
    await assert.rejects(wrapper.send({ type: "prompt", message: "continue" }), /overrides reserved built-in tool bash/);
  } finally {
    wrapper.destroy();
  }
});
...
test("fork_branch copies the selected assistant entry without replacing the source session", async () => {
  ...
  try {
    ...
    assert.equal(forked.getEntries().at(-1).customType, "pi-web:fork-cost-baseline");
    assert.equal(forked.getEntries().at(-1).parentId, selectedEntryId);
...
test("fork before the first message persists a reopenable message-free child session", async () => {
  ...
  try {
    ...
    assert.deepEqual(forked.getEntries()[0], manager.getEntry(settingsEntryId));
    assert.equal(forked.getEntries()[1].customType, "pi-web:fork-cost-baseline");
    assert.equal(forked.getEntries()[1].parentId, settingsEntryId);
  } finally {
    wrapper.destroy();
    if (forkedFile) await unlink(forkedFile);
    if (sourceFile) await unlink(sourceFile);
...
test("session replacement rejects active work and clone writes one reopenable child", async () => {
  ...
  try {
    ...
    assert.equal(cloned.getEntries().at(-1).customType, "pi-web:fork-cost-baseline");
    assert.equal(cloned.getEntries().at(-1).parentId, cloneLeafId);
