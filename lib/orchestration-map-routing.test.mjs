...
test("complete overview includes every agent and every owner's three link types", () => {
  const profiles = [
    agent("coordinator", { allowedChildren: ["reader", "writer"],
      dependencies: { writer: ["reader"] }, contextProviders: { reader: ["writer"] } }),
    agent("reader"), agent("writer"),
    agent("unconnected", { allowedChildren: ["other"] }), agent("other"),
  ];
  const graph = buildOrchestrationGraph({ profiles, main: { allowedChildren: ["coordinator"] },
    ownerId: null, layer: "all", connectedOnly: false });
  assert.equal(graph.nodes.length, profiles.length + 1);
  assert.deepEqual(graph.edges.map((edge) => [edge.kind, edge.ownerId, edge.source, edge.target]), [
    ["delegation", MAIN_NODE_ID, MAIN_NODE_ID, "coordinator"],
    ["delegation", "coordinator", "coordinator", "reader"],
    ["delegation", "coordinator", "coordinator", "writer"],
    ["dependencies", "coordinator", "reader", "writer"],
    ["contextProviders", "coordinator", "writer", "reader"],
    ["delegation", "unconnected", "unconnected", "other"],
  ]);
  const nodes = autoLayoutGraph(graph.nodes, graph.edges, null);
  assertClear(nodes, routeOrchestrationEdges(nodes, graph.edges));
});

test("full 200-agent overview retains nested links and routes around every card", () => {
  const coordinators = Array.from({ length: 10 }, (_, index) => `coordinator-${index}`);
  const profiles = coordinators.flatMap((name, group) => {
    const children = Array.from({ length: 20 }, (_, index) => `agent-${group * 20 + index}`);
    return [agent(name, { allowedChildren: children,
      dependencies: Object.fromEntries(children.slice(1, 5).map((child, index) => [child, [children[index]]])),
      contextProviders: { [children[15]]: [children[17], children[18]] },
    }), ...children.map((child) => agent(child))];
  });
  const graph = buildOrchestrationGraph({ profiles, main: { allowedChildren: coordinators },
    ownerId: null, layer: "all", connectedOnly: false });
  assert.equal(graph.nodes.length, 211);
  assert.equal(graph.edges.length, 270);
  assert.equal(graph.edges.filter((edge) => edge.kind === "dependencies").length, 40);
  assert.equal(graph.edges.filter((edge) => edge.kind === "contextProviders").length, 20);
  const nodes = autoLayoutGraph(graph.nodes, graph.edges, null);
  assertClear(nodes, routeOrchestrationEdges(nodes, graph.edges));
});
