...
test("an untrusted project profile cannot force an unbounded read during discovery", async (t) => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-large-profile-"));
  t.after(() => rm(cwd, { recursive: true, force: true }));
  const dir = join(cwd, ".pi", "agents");
  await mkdir(dir, { recursive: true });
  const file = await open(join(dir, "oversized.md"), "w");
  try { await file.truncate(64 * 1024 * 1024); } finally { await file.close(); }
  assert.throws(() => listSubagentProfiles(cwd), /limit/);
});
...
test("profile read and directory access errors cannot fall through to a lower-scope profile", async () => {
  ...
  try {
    ...
    // Inject a deterministic EACCES between readdir and open in an isolated
...
test("resource restoration distinguishes no marker from corrupt metadata and rejects privilege escalation", () => {
  ...
  assert.equal(readSubagentSessionResources(entry(base)).maxTurns, undefined,
    "existing sessions without a pinned limit retain their earlier behavior");
  assert.equal(readSubagentSessionResources(entry({ ...base, maxTurns: 3 })).maxTurns, 3);
  assert.deepEqual(readSubagentSessionResources(entry({ ...base, tools: ["github_read"] })).tools, ["github_read"],
    "a GitHub reader resumes with its pinned read-only tool and no external extensions");
  assert.throws(() => readSubagentSessionResources(entry({ ...base, tools: ["github_write"] })), /Invalid.*tools/);
  for (const maxTurns of [0, -1, 1.5, Number.POSITIVE_INFINITY, "3"]) {
    assert.throws(() => readSubagentSessionResources(entry({ ...base, maxTurns })), /Invalid.*snapshot/);
  }
...
test("a malformed hand-authored max_turns disables the profile instead of removing its limit", async () => {
  const cwd = await mkdtemp(join(tmpdir(), "pi-web-invalid-turn-limit-"));
  try {
    const profiles = join(cwd, ".pi", "agents");
    await mkdir(profiles, { recursive: true });
    for (const value of ["0.5", "not-a-number", "9007199254740992"]) {
      await writeFile(join(profiles, "capped-cost.md"),
        `---\nname: capped-cost\ndescription: Capped specialist\ntools: none\nmax_turns: ${value}\n---\nPerform a short task.\n`);
      const profile = listSubagentProfiles(cwd).find((candidate) => candidate.name === "capped-cost");
      assert.equal(profile.enabled, false, `max_turns: ${value}`);
      assert.match(profile.configurationError, /Invalid max_turns/);
      assert.equal(resolveSubagentProfile(cwd, "capped-cost"), undefined);
    }
  } finally {
    await rm(cwd, { recursive: true, force: true });
  }
...
test("saved profiles normalize runtime values and reject invalid settings", async () => {
  ...
  try {
    ...
    assert.throws(
      ...
      /Max turns must be zero.*safe turn/,
    );
    assert.throws(
    ...
    assert.throws(
      () => saveProjectSubagentProfile(cwd, profile({ maxTurns: 0.5 })),
      /Max turns must be zero.*safe turn/,
    );
  } finally {
    await rm(cwd, { recursive: true, force: true });
